		...
		function timeObjectTests() {
			describe('timeObject', () => {
				...
				it('{ hrs24: 11, hrs12: 11, min: 30, mode: AM } => false', () => {
					expect(
						is.PM.timeObject({ hrs24: 11, hrs12: 11, min: 30, mode: 'AM' }),
					).to.equal(false)
				})
				it('{ hrs24: 12, hrs12: 12, min: 0, mode: PM } => true', () => {
					expect(is.PM.timeObject({ hrs24: 12, hrs12: 12, min: 0, mode: 'PM' })).to.equal(
						true,
					)
				})
				it('{ hrs24: 13, hrs12: 1, min: 30, mode: PM } => true', () => {
					expect(is.PM.timeObject({ hrs24: 13, hrs12: 1, min: 0, mode: 'PM' })).to.equal(
						true,
					)
				})
		...
		function timeObjectTests() {
			describe('timeObject', () => {
				...
				it('{ hrs24: 11, hrs12: 11, min: 30, mode: AM } => true', () => {
					expect(
						is.AM.timeObject({ hrs24: 11, hrs12: 11, min: 30, mode: 'AM' }),
					).to.equal(true)
				})
				it('{ hrs24: 12, hrs12: 12, min: 0, mode: PM } => false', () => {
					expect(is.AM.timeObject({ hrs24: 12, hrs12: 12, min: 0, mode: 'PM' })).to.equal(
						false,
					)
				})
				it('{ hrs24: 13, hrs12: 1, min: 30, mode: PM } => false', () => {
					expect(is.AM.timeObject({ hrs24: 13, hrs12: 1, min: 0, mode: 'PM' })).to.equal(
						false,
					)
				})
				it('{ hrs24: --, hrs12: --, min: --, mode: -- } => false', () => {
...
function isTimeObjectTests() {
	describe('is timeObject', () => {
		...
		it('{hrs: 12, hrs24: 0, hrs12: 12, min: 0, mode: AM} => false', () => {
			expect(is.timeObject({ hrs: 12, hrs24: 0, hrs12: 12, min: 0, mode: 'AM' })).to.equal(
				false,
			)
		})
